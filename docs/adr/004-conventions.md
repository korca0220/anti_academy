...
- **원칙**: Flutter 위젯은 1파일 1위젯 클래스를 따른다.
- 화면 파일에는 대표 화면 위젯만 둔다. 예: `ProfileViewScreen`
- 보조 위젯은 `widgets/` 디렉토리나 같은 기능의 적절한 하위 디렉토리로 분리한다.
- **`_` private 위젯도 예외 없이 `widgets/`로 분리한다.** `_` prefix는 파일 분리 면제 근거가 되지 않는다.
- 예외: 없음. 커밋 전 반드시 1파일 1클래스를 확인한다.
