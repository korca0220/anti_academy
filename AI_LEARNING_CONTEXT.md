...
## 🔄 Implementation Workflow (구현 워크플로우)

스프린트 시작 시 **모든 TODO 뼈대를 먼저 작성**한 뒤, 스텝 단위로 반복한다.

```
[스프린트 시작]
  └─ AI가 전체 TODO 뼈대 파일 생성 (로직 없음, TODO 주석만)

[스텝 반복]
  1. TODO 확인 — 이번 스텝에서 채울 항목 확인
  2. 구현 — 사용자가 직접 로직 작성
  3. 검증 — 실행 또는 테스트로 동작 확인
  4. TODO 제거 — 완료된 TODO 주석 삭제
  5. 커밋 — type(scope): message 형식으로 커밋
```

**역할 분담:**
- 사용자: TODO 로직 구현
- AI: 구현 확인 → 검증(analyze/논리 검토) → TODO 주석 제거 → 커밋

**규칙:**
- AI는 뼈대(Shell)만 제공하고, 로직 구현은 사용자가 직접 한다.
- 사용자가 구현을 완료했다고 하면, AI가 나머지(검증 → TODO 제거 → 커밋)를 자동으로 처리한다.
- TODO 주석이 남아있으면 커밋하지 않는다.
- 검증 없이 다음 스텝으로 넘어가지 않는다.

**뼈대 생성 전 체크리스트:**
- [ ] 파일당 위젯 클래스가 1개인가? (`_` private 위젯도 예외 없이 `widgets/`로 분리)
- [ ] 새 위젯 파일은 `presentation/widgets/`에 위치하는가?

---

## 🪜 Step Presentation Format (스텝 표시 규칙)

스프린트 작업 중 스텝 진행 상황은 다음 형식으로 표시합니다.
...
**규칙:**
- 스프린트 시작 시 전체 스텝 목록을 한 번 보여준다.
- 각 스텝 완료 후 업데이트된 목록을 다시 출력한다.
- 현재 진행 중인 스텝은 박스(`┌─┐`)로 강조한다.
- 완료된 스텝은 `✅`, 진행 중은 `🔨`, 대기 중은 번호만 표시.
